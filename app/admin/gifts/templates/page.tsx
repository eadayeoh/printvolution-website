  ...
  return (
    <div className="p-6">
      ...
      {templates.length === 0 ? (
        ...
        (() => {
          // Bucket templates by group_name; null/empty falls into "Ungrouped".
          const grouped = new Map<string, typeof templates>();
          for (const t of templates) {
            const key = (t.group_name ?? '').trim() || 'Ungrouped';
            if (!grouped.has(key)) grouped.set(key, []);
            grouped.get(key)!.push(t);
          }
          // Stable sort: named groups alphabetically first, Ungrouped last.
          ...
          return (
            <div className="space-y-8">
              {keys.map((group) => {
                ...
                return (
                  <section key={group}>
                    <div className="mb-3 flex items-baseline gap-2 border-b border-neutral-200 pb-2">
                      <h2 className="text-sm font-bold text-ink">{group}</h2>
                      <span className="text-[10px] text-neutral-500">{list.length} template{list.length === 1 ? '' : 's'}</span>
                    </div>
